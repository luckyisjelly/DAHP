...
- ✅ 체크인 주기 변경 즉시 반영
- ✅ overdue 정확히 판단
- ✅ overdue이면 INACTIVITY_PERIOD 규칙은 다음 스케줄러 실행 시 자동 트리거 대상 (시나리오 6)
...
## 7. 시나리오 6: 자동 조건 평가 (INACTIVITY)

### 목표
스케줄러가 사용자 체크인 만료를 감지하면 자동으로 인계 트리거 발생.

### 사전
- 김민준이 INACTIVITY_PERIOD 조건(예: 30일)의 규칙을 ACTIVE 상태로 설정
- 자산 1개 + 수령인 1명 포함
- `lastCheckInAt`이 31일 전

### 단계

| # | 행위자 | 행동 | 시스템 응답 |
|---|---|---|---|
| 1 | (시스템 스케줄러) | 매시간/매일 실행 | `@Scheduled` |
| 2 | 스케줄러 | INACTIVITY 규칙들 조회 | DB 쿼리 |
| 3 | 스케줄러 | 김민준의 `lastCheckInAt` 검사 → 31일 경과 확인 | overdue |
| 4 | 스케줄러 | 규칙 trigger (수동 trigger와 동일 로직) | TRIGGERED, eventCount=1 |
| 5 | 시스템 | 콘솔 알림 출력 | `[NOTIFY] 수령인에게 토큰 발송: <token>` |
| 6 | 수령인 | 토큰으로 자산 조회 | 200, 자산 내용 |

### 시연 어려움
30일 비활성을 시연에서 기다릴 수 없음. 다음 중 택1:
- **(A) 시연용 프로파일**: `application-demo.yml`로 스케줄러 주기를 1분, INACTIVITY 조건을 1분으로 변경 → 시연 시 체크인 안 하고 1분 기다리면 자동 트리거
- **(B) 수동 trigger로 시연**: INACTIVITY 규칙도 manual trigger API로 강제 트리거 가능 (모든 ACTIVE 규칙 대상). 자동 평가는 코드와 테스트로만 보여줌
- **(C) DB 직접 조작**: 시연 직전에 `lastCheckInAt`을 일부러 과거로 UPDATE

권장: **(B)** — 시연 중 흐름 가장 안정적, 자동 평가는 테스트 코드와 스케줄러 로그로 입증.
...
- 조건 타입 추가: PERIODIC_CHECK_FAILED, EMERGENCY_REQUEST
...
|---|---|
| 2026-05-13 | 초안 작성 |
| 2026-05-13 | INACTIVITY 자동 평가 시나리오 추가 (시나리오 5), 시연 전략 명시 |
